...
function ACTPage() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('http://127.0.0.1:5000/api/ACT?type=text', { act_problem: problem });
  ...
  const captureProblem = async () => {
    try {
      const response = await axios.post('http://127.0.0.1:5000/api/ACT?type=capture');
      setResult(response.data.result);
    } catch (error) {
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit}>
        <label>Type your ACT problem:</label>
        <input
          type="text"
          value={problem}
          onChange={(e) => setProblem(e.target.value)}
          placeholder="e.g., Any Math, Reading or Writing problem."
        />
