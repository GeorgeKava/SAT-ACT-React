...
function MathPage() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('http://127.0.0.1:5000/api/math?type=text', { math_problem: problem });
  ...
  const captureProblem = async () => {
    try {
      const response = await axios.post('http://127.0.0.1:5000/api/math?type=capture');
      setResult(response.data.result);
    } catch (error) {
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit}>
        <label>Type your math problem:</label>
        <input
          type="text"
          value={problem}
          onChange={(e) => setProblem(e.target.value)}
          placeholder="e.g., Solve x^2 + 2x - 8 = 0"
        />
